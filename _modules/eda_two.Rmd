...
first command and print. Notice how the function chain is the mostly
the same as above, but (1) written in a single linked chain and (2)
using a different `geom_*()` command at the end to indicate that we
...
third dimension. Specifically, we want to change the point color based
on whether a student plans to earn a Bachelor's degree or
