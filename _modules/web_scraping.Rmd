...
[RSelenium](https://github.com/ropensci/RSelenium) and some knowledge
of [Javascript](https://www.w3schools.com/js/default.asp) is helpful.

For this module, however, we'll read static web tables from [NCES
Digest of Education
...
rid of the commas. The `gsub()` function is perfect for this. It's
commands take the form: `gsub('<regular expression to remove>',
'<regular expression to substitute in place of old>',
<variable>)`. Regular expressions can become complicated, but our use
here is simple:  
...
(always extra 3 as the 5th digit), we can fix it using `substring()`.  
...
convert the years to a data format.
...
After a small dip in the early 1970s (once the Baby Boom generation
had largely left college), enrollment trends have steadily risen over
time.  
