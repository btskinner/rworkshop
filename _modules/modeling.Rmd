...
One simple common statistical test is a t-test for a difference in
means across groups (there are, of course, [others and R can compute
them](https://www.rdocumentation.org/packages/stats/versions/3.4.3/topics/t.test)). This
can be computed using the R formula syntax: `y ~ x`, meaning, in this
case "base-year math scores against mother's college education
...
```
Linear models are the bread and butter of many data analysts. In R,
the `lm()` command is used to compute an OLS regression. Unlike above,
where we just let the `t.test()` output print to the console, we can
and will store the output in an object.
...
we've use before except add all the terms you want to add to the
right-hand side separated by plus `+` signs.
...
> (`x`). Compare these values the math scores in the data frame.
...
`I()` function, which let's you raise the term to the power you want
...
`family`. Set the argument to `binomial()` for a binary output. By
default, the `link` function is a [logit](https://en.wikipedia.org/wiki/Logit).
...
predictions. We can create one, however. Though these data aren't
real, we can use them to return marginal predictions. 
...
different value, 0 and 1. All other columns take the average value of
the values in the model matrix.
